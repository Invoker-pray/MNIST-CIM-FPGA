  ...
  initial begin
    ...
    for (i = 0; i < OUT_DIM; i = i + 1) begin
      if (fc1_acc_all[i] !== ref_fc1_acc_mem[i]) begin
        $display("ERROR idx=%0d got=%0d expected=%0d", i, fc1_acc_all[i], ref_fc1_acc_mem[i]);
    ...
    if (error_count == 0)
      $display(
          "PASS: fc1_multi_block_parallel matches first %0d outputs of fc1_acc_0.hex.", OUT_DIM
      );
