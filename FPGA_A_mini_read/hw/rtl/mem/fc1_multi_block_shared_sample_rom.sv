// fc1_multi_block_shared_sample_rom.sv
// 新增 sample_rom / weight / bias 写口透传

module fc1_multi_block_shared_sample_rom #(
    parameter int PAR_OB   = 1,
    parameter int N_SAMPLES = 20
) (
    input logic clk,
    input logic rst_n,
    input logic start,
    input logic [$clog2(N_SAMPLES)-1:0] sample_id,
    input logic [((mnist_cim_pkg::N_OUTPUT_BLOCKS > 1) ? $clog2(
mnist_cim_pkg::N_OUTPUT_BLOCKS
) : 1)-1:0] base_ob,

    output logic busy,
    output logic done,

    output logic signed [mnist_cim_pkg::PSUM_WIDTH-1:0]
        fc1_acc_all [0:PAR_OB*mnist_cim_pkg::TILE_OUTPUT_SIZE-1],

    // ── Write port: mnist_sample_rom ──────────────────────────────────────
    input logic        samp_wr_en,
    input logic [31:0] samp_wr_data,

    // ── Write port: fc1_weight_bank (广播给所有 PAR_OB 实例) ──────────────
    input logic        fc1w_wr_en,
    input logic [31:0] fc1w_wr_data,

    // ── Write port: fc1_bias_bank (广播给所有 PAR_OB 实例) ────────────────
    input logic        fc1b_wr_en,
    input logic [31:0] fc1b_wr_data
);
  import mnist_cim_pkg::*;

  localparam int OB_SEL_W = (N_OUTPUT_BLOCKS > 1) ? $clog2(N_OUTPUT_BLOCKS) : 1;

  typedef enum logic [2:0] {
    S_IDLE  = 3'd0,
    S_CLEAR = 3'd1,
    S_READ  = 3'd2,
    S_WAIT  = 3'd3,
    S_ACCUM = 3'd4,
    S_DONE  = 3'd5
  } state_t;

  state_t state, state_n;

  logic [$clog2(N_INPUT_BLOCKS)-1:0] ib, ib_n;
  logic clear_psum, en_psum;

  logic signed [INPUT_WIDTH-1:0] x_tile     [0:TILE_INPUT_SIZE-1];
  logic        [X_EFF_WIDTH-1:0] x_eff_tile [0:TILE_INPUT_SIZE-1];

  logic signed [PSUM_WIDTH-1:0]  fc1_acc_block [0:PAR_OB-1][0:TILE_OUTPUT_SIZE-1];

  mnist_sample_rom #(
      .N_SAMPLES(N_SAMPLES)
  ) u_mnist_sample_rom (
      .clk       (clk),
      .rst_n     (rst_n),
      .sample_id (sample_id),
      .ib        (ib),
      .x_tile    (x_tile),
      .x_eff_tile(x_eff_tile),
      .wr_en     (samp_wr_en),
      .wr_data   (samp_wr_data)
  );

  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      state <= S_IDLE;
      ib    <= '0;
    end else begin
      state <= state_n;
      ib    <= ib_n;
    end
  end

  always_comb begin
    state_n    = state;
    ib_n       = ib;
    clear_psum = 1'b0;
    en_psum    = 1'b0;
    busy       = 1'b0;
    done       = 1'b0;

    case (state)
      S_IDLE: begin
        ib_n = '0;
        if (start) state_n = S_CLEAR;
      end
      S_CLEAR: begin
        busy       = 1'b1;
        clear_psum = 1'b1;
        ib_n       = '0;
        state_n    = S_READ;
      end
      S_READ: begin
        busy    = 1'b1;
        state_n = S_WAIT;
      end
      S_WAIT: begin
        busy    = 1'b1;
        state_n = S_ACCUM;
      end
      S_ACCUM: begin
        busy    = 1'b1;
        en_psum = 1'b1;
        if (ib == N_INPUT_BLOCKS - 1) begin
          state_n = S_DONE;
          ib_n    = ib;
        end else begin
          ib_n    = ib + 1'b1;
          state_n = S_READ;
        end
      end
      S_DONE: begin
        done = 1'b1;
        if (!start) state_n = S_IDLE;
      end
      default: begin
        state_n    = S_IDLE;
        ib_n       = '0;
        clear_psum = 1'b0;
        en_psum    = 1'b0;
        busy       = 1'b0;
        done       = 1'b0;
      end
    endcase
  end

  genvar g_ob, g_idx;
  generate
    for (g_ob = 0; g_ob < PAR_OB; g_ob = g_ob + 1) begin : GEN_OB_ENGINE
      logic [OB_SEL_W-1:0] ob_sel;
      assign ob_sel = base_ob + g_ob[OB_SEL_W-1:0];

      // weight/bias 写口广播给所有 ob_engine 实例
      fc1_ob_engine_shared_input u_fc1_ob_engine_shared_input (
          .clk          (clk),
          .rst_n        (rst_n),
          .clear_psum   (clear_psum),
          .en_psum      (en_psum),
          .ib           (ib),
          .ob_sel       (ob_sel),
          .x_eff_tile   (x_eff_tile),
          .fc1_acc_block(fc1_acc_block[g_ob]),
          .fc1w_wr_en   (fc1w_wr_en),
          .fc1w_wr_data (fc1w_wr_data),
          .fc1b_wr_en   (fc1b_wr_en),
          .fc1b_wr_data (fc1b_wr_data)
      );

      for (g_idx = 0; g_idx < TILE_OUTPUT_SIZE; g_idx = g_idx + 1) begin : GEN_PACK
        assign fc1_acc_all[g_ob*TILE_OUTPUT_SIZE+g_idx] = fc1_acc_block[g_ob][g_idx];
      end
    end
  endgenerate

endmodule
